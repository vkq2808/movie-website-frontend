...
export default function AddToListButton({ movieId }: { movieId: string }) {
  ...
  const [creatingVisibility, setCreatingVisibility] = useState<'PRIVATE' | 'PUBLIC'>('PRIVATE');
    ...
    movieListApi
      ...
      .then((res) => {
        ...
        const items = (res || []).map((l: any) => ({
          ...l,
          checked: !!(l.items || []).find((it: any) => it.movie?.id === movieId),
        }));
        setLists(items);
      })
  ...
  const toggle = async (listId: string, checked: boolean) => {
    ...
    try {
      ...
      // update local UI
      setLists((prev) => prev.map((l) => (l.id === listId ? { ...l, checked: !checked } : l)));
  ...
  const handleCreateAndAdd = async () => {
    if (!ensureAuth()) return;
    if (!creatingName || creatingName.trim().length === 0) return toast.error('Name is required');
    ...
    <Dialog open={open} onOpenChange={(v) => {
      ...
      <DialogContent>
        ...
        <div className="mt-2 grid grid-cols-1 gap-4">
          <div>
            <div className="text-sm font-medium mb-2">Your playlists</div>
            <div className="h-48">
              <ScrollArea className="h-full">
                <div className="flex flex-col gap-2 p-1">
                  ...
                  {!loading && lists.length === 0 && (
                    <div className="text-sm text-gray-600">You don&apos;t have any playlists yet.</div>
                  )}
